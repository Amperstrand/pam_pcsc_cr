/*
 * ntag424_verifier.c — NTAG424 DNA cryptographic verification.
 *
 * Implements the SUN (Secure Unique Number) verification flow:
 *   - Extract p= and c= parameters from URLs or NDEF messages
 *   - Decrypt the p parameter with K1 to recover UID + SDM counter
 *   - Verify the truncated CMAC (c parameter) against K2
 *   - Derive K1/K2 from IssuerKey using Bolt Card algorithm
 *
 * Uses the project's crypto.h abstraction (AES-CBC with zero IV)
 * for all AES operations.  CMAC (RFC 4493, single-block variant)
 * is implemented here because crypto.h does not expose CMAC.
 *
 * No hardware, PC/SC, or PAM dependencies — pure crypto.
 */
